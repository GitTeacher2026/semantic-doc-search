...
- **محرر PDF** (مثل Sejda / Acrobat): نص، تمييز، محو، رسم، توقيع، صور، تدوير، دمج، OCR، وتنزيل — من صفحة **الملفات**
- **مستكشف ملفات** منفصل (شبكة/قائمة، تصفح حسب المصدر والتصنيف، بحث وترتيب)
- تخزين مشفّر على **MEGA** (اختياري) أو **Google Drive** أو **OneDrive** أو محلياً
...
**المستندات والملفات** يمكن تخزينها في **MEGA** عند الحاجة. الاتصال بـ MEGA اختياري بعد تسجيل الدخول — اربطه من صفحة الرفع فقط إذا أردت الرفع أو مزامنة/عرض ملفات MEGA.

عند الاتصال بـ MEGA لأول مرة، يُنقل الفهرس القديم من GitHub تلقائياً إلى MEGA (إن وُجد).

| المصدر | الإعداد | الملاحظات |
|--------|---------|-----------|
| **MEGA** | بريد وكلمة مرور في الواجهة (اختياري) | تخزين المستندات والملفات عند الاتصال |
...
يُفتح العرض في نافذة منبثقة مع ترجمة سريعة إلى العربية (Google Translate + Lingva أولاً؛ MyMemory احتياطي أخير فقط؛ LibreTranslate اختياري). عرض الإنجليزية LTR. جداول/صور النشرة تُحفظ كنص منظم أو كعنصر `[Image]` بدل إسقاط القسم.
DailyMed يحمّل نسخة XSL/الطباعة الكاملة وليس معاينة قصيرة.

## نصوص الموقع (للمسؤول)

بعد تسجيل الدخول كمسؤول يظهر زر **نصوص الموقع**. منه يمكن تعديل أو إخفاء النصوص التوضيحية في الصفحات (مثل مقدمة بحث الشهادات، تلميحات MEGA، ومقدمات SmPC/الدساتير) دون انتظار تعديل برمجي.

تُحفظ القيم في `data/site-copy.json` عبر GitHub Contents API.

## بحث شهادات ISO

تبويب **البحث → شهادات ISO / Certification** يبحث عبر أشهر هيئات التصديق بدل الاعتماد الكامل على IAF CertSearch:

- SGS (نتائج مباشرة من دليل الشهادات العام)
- BSI، Bureau Veritas، TÜV SÜD، TÜV Rheinland (Certipedia)، Intertek، DNV، LRQA، DEKRA، UKAS CertCheck

أدخل اسم الشركة و/أو رقم الشهادة، واختر معايير ISO والهيئات، ثم افتح الأدلة الرسمية للتحقق داخل موقع كل جهة.
