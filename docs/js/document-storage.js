...
export function inferDocumentStorage(doc) {
  ...
  if (backends.includes(STORAGE_BACKENDS.MEGA)) return STORAGE_BACKENDS.MEGA;
...
export function getStorageBackendLabel(backend) {
  ...
  if (backend === STORAGE_BACKENDS.GITHUB) return "GitHub (قديم)";
  return "محلي";
}
...
export function getDocumentStoragePath(doc) {
  ...
  if (doc?.fileData) {
    paths.push(`فهرس قديم (GitHub) / ${category} / ${filename}`);
  }
  if (!paths.length) {
